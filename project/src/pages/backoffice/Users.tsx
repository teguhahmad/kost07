...
const BackofficeUsers: React.FC = () => {
  ...
  const loadUsers = async () => {
    try {
      ...
      // Get current user's role using maybeSingle()
      ...
      // Handle case where user is not found in backoffice_users
      if (!currentUser) {
        setError('You do not have permission to access the backoffice. Please contact an administrator.');
        return;
      }
      ...
      if (currentUser.role !== 'superadmin') {
        setError('Only superadmin users can access this page.');
        return;
      }
      ...
      const { data, error: fetchError } = await supabase
        .from('backoffice_users')
        .select('id, email, role, name, created_at, status, last_login')
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (currentUser.status !== 'active' || currentUser.role !== 'superadmin') {
        setError('Only active superadmin users can manage users.');
        return;
      }
